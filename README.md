...
The module sets a direct method handler to allow the DownloadBlobs method to be executed, which expect a `List<BlobInfo>` as the payload representing the list of blobs to be downloaded. The `BlobInfo` class is defined as follows:
...
Then, the back-end application invokes the direct method, directly by using the `DeployBlobsAsync()`, or by creating an scheduled job using the `ScheduleDeployBlobsJobAsync()` (both methods are available in the IoTEdgeDeployBlobs.Sdk library).

Once the `DownloadBlobs` is executed (directly or using a scheduled job), the content of `publishednodes.json` will be downloaded to the module folder `/app/blobs/opcPublisher`, so at the host, the file `/etc/iotedge/deployBlobs/opcPublisher/publishednodes.json` will be created.
...
Under /source/IoTEdgeDeployBlob.SampleConsole there is a console utility to demostrate the usage of the Sdk to create the requests and manage the responses. 

The Sample console requires the following configuration:

```json
//Ensure you have a Shared Access Policy that allows Service Connect and Registry Read & Write to setup the IoT Hub Connection String.
{
...
The way other containers reload the downloaded content is out of the scope right now. It can be just a restart of the module that can be done via DM call to edgeAgent eventually.
